...
export class JwtAuthGuard extends AuthGuard('jwt') {
  ...
  canActivate(context: ExecutionContext): boolean | Promise<boolean> | Observable<boolean> {
    ...
    this.logger.debug(`JWT guard checking request to: ${request.method} ${request.url}`);
    ...
    this.logger.debug(`Auth header format: ${authHeader.substring(0, 10)}... (length: ${authHeader.length})`);
  ...
  handleRequest(err, user, info) {
    ...
    if (!user) {
      this.logger.error(`JWT validation failed: ${info?.message || 'No user found'}`);
