...
import { CmsContent, ContentStatus, ContentType, ContentSection } from './cms-content.entity';
...
export class CmsService {
    ...
    async findAll(filters?: any): Promise<CmsContent[]> {
        ...
        if (filters) {
            if (filters.contentType) {
                query.andWhere('cms.contentType = :contentType', { contentType: filters.contentType });
            }
            if (filters.section) {
                query.andWhere('cms.section = :section', { section: filters.section });
            }
            if (filters.status) {
                query.andWhere('cms.status = :status', { status: filters.status });
            }
        }
    ...
    async findByKey(contentKey: string): Promise<CmsContent> {
        const content = await this.cmsContentRepository.findOne({ where: { contentKey } });
        if (!content) {
            throw new NotFoundException(`CMS content with key ${contentKey} not found`);
        }
        return content;
    }
    ...
    async update(id: number, contentData: Partial<CmsContent>): Promise<CmsContent> {
    ...
    async remove(id: number): Promise<void> {
        ...
        if (content.cloudinaryPublicId) {
            try {
                await this.cloudinaryService.deleteImage(content.cloudinaryPublicId);
            } catch (error) {
    ...
    async uploadImage(file: Express.Multer['File'], folder: string = 'cms'): Promise<any> {
