...
export class DashboardService {
    ...
    async getSalesData(period: string = 'week') {
        try {
            ...
            if (period === 'year') {
                ...
                return {
                    dates: Array.from({length: daysInMonth}, (_, i) => `${(i+1).toString().padStart(2, '0')}/${(now.getMonth() + 1).toString().padStart(2, '0')}`),
    ...
    async getOrderStatuses() {
        try {
            ...
            if (statusesData && statusesData.length > 0) {
                ...
                const statuses = statusesData.map(item => statusMap[item.status] || item.status);
    ...
    async getCustomerGrowth(period: string = 'year') {
        try {
            ...
            this.logger.error(`Error getting customer growth: ${error.message}`);
            return { dates: [], counts: [] };
        }
    }
    ...
    async getTopSellingProducts(limit: number = 10, period: string = 'month') {
        try {
            ...
            this.logger.error(`Error getting top selling products: ${error.message}`);
            return { products: [] };
        }
    }
    ...
    private generateDateRange(startDate: Date, endDate: Date, period: string) {
        ...
        while (currentDate <= endDate) {
            dates.push(`${currentDate.getDate().toString().padStart(2, '0')}/${(currentDate.getMonth() + 1).toString().padStart(2, '0')}`);
            counts.push(0);
