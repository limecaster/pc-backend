...
import { CreateDiscountDto, UpdateDiscountDto, DiscountResponseDto, DiscountStatisticsDto } from './dto/discount.dto';
...
export class DiscountService {
    ...
    async create(createDiscountDto: CreateDiscountDto): Promise<DiscountResponseDto> {
        ...
        if (existingDiscount) {
            throw new ConflictException(`Discount code ${createDiscountDto.discountCode} already exists`);
        }
        ...
        if (createDiscountDto.type === 'percentage' && (createDiscountDto.discountAmount < 0 || createDiscountDto.discountAmount > 100)) {
    ...
    async update(id: number, updateDiscountDto: UpdateDiscountDto): Promise<DiscountResponseDto> {
        ...
        if (updateDiscountDto.discountCode && updateDiscountDto.discountCode !== discount.discountCode) {
        ...
        // For percentage type, ensure amount is between 0 and 100
        if (updateDiscountDto.type === 'percentage' && 
            updateDiscountDto.discountAmount !== undefined && 
            (updateDiscountDto.discountAmount < 0 || updateDiscountDto.discountAmount > 100)) {
            throw new BadRequestException('Percentage discount must be between 0 and 100');
        }
        ...
        if (updateDiscountDto.targetType) {
            this.validateTargetFields({
                ...
                categoryNames: updateDiscountDto.categoryNames || discount.categoryNames,
                customerIds: updateDiscountDto.customerIds || discount.customerIds,
            });
        }
    ...
    async remove(id: number): Promise<{ success: boolean; message: string }> {
        ...
        return { success: true, message: `Discount ${discount.discountCode} has been deleted` };
    }
    ...
    async getStatistics(): Promise<DiscountStatisticsDto> {
        try {
            ...
            const totalUsage = discounts.reduce((sum, discount) => sum + discount.usageCount, 0);
            const totalSavings = discounts.reduce((sum, discount) => sum + Number(discount.totalSavingsAmount || 0), 0);
    ...
    private validateTargetFields(discountDto: any): void {
        const { targetType, productIds, categoryNames, customerIds } = discountDto;
        
        // Check if appropriate target IDs are provided based on targetType
        switch(targetType) {
            case 'products':
                if (!productIds || !Array.isArray(productIds) || productIds.length === 0) {
                    throw new BadRequestException('Products must be specified when target type is "products"');
                }
                break;
            case 'categories':
                if (!categoryNames || !Array.isArray(categoryNames) || categoryNames.length === 0) {
                    throw new BadRequestException('Categories must be specified when target type is "categories"');
                }
                break;
            case 'customers':
                if (!customerIds || !Array.isArray(customerIds) || customerIds.length === 0) {
                    throw new BadRequestException('Customers must be specified when target type is "customers"');
                }
                break;
        }
    }
    ...
    async getAutomaticDiscounts(options?: {
        ...
        try {
            ...
            const queryBuilder = this.discountRepository.createQueryBuilder('discount')
                ...
                .andWhere('discount.isAutomatic = :isAutomatic', { isAutomatic: true })
            ...
            const applicableDiscounts = allAutomaticDiscounts.filter(discount => {
                // Check minimum order amount
                if (discount.minOrderAmount && options?.orderAmount && 
                    options.orderAmount < discount.minOrderAmount) {
                    return false;
                }
                
                // Check first purchase restriction
                if (discount.isFirstPurchaseOnly && options?.isFirstPurchase === false) {
                    return false;
                }
                ...
                if (discount.targetType !== 'all') {
                    switch(discount.targetType) {
                        case 'products':
                            if (!options?.productIds || !discount.productIds?.some(id => 
                        ...
                        case 'categories':
                            if (!options?.categoryNames || !discount.categoryNames?.some(cat => 
                        ...
                        case 'customers':
                            if (!options?.customerId || !discount.customerIds?.includes(options.customerId)) {
                                return false;
                            }
                            break;
                    }
            ...
            return applicableDiscounts.map(discount => this.mapToDto(discount));
        } catch (error) {
            this.logger.error(`Error getting automatic discounts: ${error.message}`);
            return [];
        }
    }

    // Add method to get best discount for multiple options
    async getBestDiscount(discounts: Discount[], productPrice: number): Promise<{
        ...
        const discountsWithAmount = discounts.map(discount => {
            ...
            if (discount.type === 'percentage') {
                ...
                amount = Math.min(Number(discount.discountAmount), productPrice);
            }
    ...
    async getTotalAutoDiscountAmount(discounts: Discount[], orderAmount: number): Promise<number> {
        let total = 0;
    ...
    async validateDiscount(
        ...
        try {
            ...
            if (foundDiscount.status !== 'active' || 
                foundDiscount.startDate > today ||
                foundDiscount.endDate < today) {
                return { valid: false, errorMessage: 'This discount code is not active or has expired' };
            }

            // If it's a product-specific discount, at least one product in cart must match
            if (foundDiscount.targetType === 'products' && foundDiscount.productIds && 
                foundDiscount.productIds.length > 0 && productIds && productIds.length > 0) {
                
                // Check if at least one product in cart is in the target list
                const hasMatchingProduct = productIds.some(id => foundDiscount.productIds.includes(id));
                
                if (!hasMatchingProduct) {
                    return { 
                        valid: false, 
                        errorMessage: 'This discount code is not applicable to any products in your cart'
                    };
                }
            }

            // Check minimum order amount if applicable - for overall discounts only
            if (foundDiscount.targetType === 'all' && foundDiscount.minOrderAmount && 
            ...
            if (foundDiscount.targetType === 'products' && productIds && productIds.length > 0) {
                // For product-specific discounts, only apply to matching products
                if (foundDiscount.productIds && foundDiscount.productIds.length > 0) {
                    // Use product prices for more accurate calculation
                    if (productPrices && Object.keys(productPrices).length > 0) {
                        manualDiscountAmount = this.calculateDiscountWithProductPrices(
                            foundDiscount,
                            productIds,
                            productPrices
                        );
                        ...
                        const applicableAmount = this.calculateApplicableAmountForProducts(
                            productIds,
                        ...
                        if (foundDiscount.type === 'percentage') {
                            manualDiscountAmount = (applicableAmount * Number(foundDiscount.discountAmount)) / 100;
                        } else {
                            manualDiscountAmount = Math.min(Number(foundDiscount.discountAmount), applicableAmount);
                        }
                    }
                }
            } else {
                ...
                if (foundDiscount.type === 'percentage') {
                    manualDiscountAmount = (orderAmount * Number(foundDiscount.discountAmount)) / 100;
                } else {
                    manualDiscountAmount = Math.min(Number(foundDiscount.discountAmount), orderAmount);
                }
            }
            ...
            if (productIds && productIds.length > 0) {
                ...
                if (automaticDiscounts.length > 0) {
                    ...
                    automaticDiscountAmount = await this.calculateTotalTargetedDiscounts(
                        automaticDiscounts,
                        productIds,
                        orderAmount,
                        productPrices
            ...
            const betterDiscountType = manualDiscountAmount >= automaticDiscountAmount ? 'manual' : 'automatic';
            ...
            return { 
                ...
                betterDiscountType,
                totalDiscountAmount: Math.max(manualDiscountAmount, automaticDiscountAmount)
            };
        } catch (error) {
    ...
    private async calculateTotalTargetedDiscounts(
        ...
        for (const discount of discounts) {
            ...
            if (discount.targetType === 'products' && discount.productIds && discount.productIds.length > 0) {
                const matchingProductIds = productIds.filter(id => discount.productIds?.includes(id));
                ...
                if (productPrices && Object.keys(productPrices).length > 0) {
                    ...
                    if (discount.type === 'percentage') {
                        total += (productSubtotal * discount.discountAmount) / 100;
                    } else {
                        total += Math.min(discount.discountAmount, productSubtotal);
                    }
                } else {
                    // Fall back to proportion-based calculation
                    const applicableAmount = this.calculateApplicableAmountForProducts(
                    ...
                    if (discount.type === 'percentage') {
                        total += (applicableAmount * discount.discountAmount) / 100;
                    } else {
                        total += Math.min(discount.discountAmount, applicableAmount);
                    }
                }
            } else {
    ...
    private calculateDiscountWithProductPrices(
        ...
        if (discount.targetType === 'products' && discount.productIds) {
            ...
            for (const productId of applicableProductIds) {
                ...
                if (productPrice > 0) {
                    if (discount.type === 'percentage') {
                        totalDiscount += (productPrice * Number(discount.discountAmount)) / 100;
                    } else {
                        // For fixed discount, apply to each eligible product (up to product price)
                        totalDiscount += Math.min(Number(discount.discountAmount), productPrice);
                    }
                }
            }
        } else {
            ...
            if (discount.type === 'percentage') {
                totalDiscount = (totalPrice * Number(discount.discountAmount)) / 100;
            } else {
                totalDiscount = Math.min(Number(discount.discountAmount), totalPrice);
            }
        }
    ...
    async incrementUsageCount(
        ...
        try {
            this.logger.log(`Incrementing usage count for discount #${discountId} with savings amount ${savingsAmount}`);
            ...
            discount.totalSavingsAmount = Number(discount.totalSavingsAmount) + savingsAmount;

            // Save updated discount
            return await this.discountRepository.save(discount);
        } catch (error) {
            this.logger.error(`Error incrementing discount usage: ${error.message}`);
            throw error;
        }
    }
}
