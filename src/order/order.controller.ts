...
export class OrderController {
    ...
    async trackOrder(@Param('identifier') identifier: string, @Request() req, @Query('email') email?: string) {
        ...
        if (this.recentRequests.has(requestId)) {
            ...
            if (now - timestamp < 2000) {
                this.logger.warn(`Potential duplicate request detected for order ${identifier} from IP ${clientIp}`);
            }
        }
        ...
        try {
            ...
            this.logger.log(`Tracking order ${identifier}, User authenticated: ${isAuthenticated}, Email provided: ${!!email}`);
            ...
            if (isNumericId) {
                ...
                order = await this.orderService.findOrderWithItems(parseInt(identifier));
            } else {
            ...
            if (isAuthenticated) {
                ...
                hasFullAccess = await this.orderService.checkOrderTrackingPermission(order.id, userId);
            ...
            return {
                success: false,
                message: error instanceof NotFoundException 
                    ? 'Order not found' 
     ...
     */
    @Post('track')
    async trackOrderWithVerification(@Body() body: { orderId: number | string, verificationData: string }) {
        try {
            this.logger.log(`Verifying access to order ${body.orderId} with verification data`);
            
            let order;
            let orderId = body.orderId;
            
            // Check if the order identifier is a number or order number
            if (typeof body.orderId === 'number' || /^\d+$/.test(body.orderId.toString())) {
                // Convert to number if it's a string numeric value
                const numericId = typeof body.orderId === 'string' ? parseInt(body.orderId) : body.orderId;
                order = await this.orderService.findOrderWithItems(numericId);
                orderId = numericId;
            } else {
                // Treat as order number
                order = await this.orderService.findOrderByNumber(body.orderId.toString());
            ...
            const hasAccess = await this.orderService.verifyOrderAccess(order.id, body.verificationData);
            ...
            const trackingInfo = await this.orderService.getOrderTrackingInfo(orderId, false);
    ...
    async requestTrackingOtp(@Body() body: SendOTPDto) {
        ...
        try {
            // Generate OTP
            const otp = await this.orderService.generateTrackingOTP(orderId, email);
            
            // Send OTP via email
            await this.emailService.sendOrderTrackingOTP(email, otp, orderId.toString());
            ...
            this.logger.error(`Error generating tracking OTP: ${error.message}`);
            
            // Don't leak information about which part failed
            return {
                success: false,
                message: 'If the order exists and the email is correct, a verification code has been sent.'
            };
        }
    }
    ...
    async verifyTrackingOtp(@Body() body: VerifyOTPDto) {
        ...
        try {
            // Verify OTP
            const isValid = await this.orderService.verifyTrackingOTP(orderId, email, otp);
    ...
    private recordOtpAttempt(key: string): void {
        const now = Date.now();
        const attempt = this.otpAttempts.get(key) || { count: 0, lastAttempt: now };
    ...
    private isRateLimited(key: string): boolean {
        ...
        const now = Date.now();
        if (now - attempt.lastAttempt > 3600000) { // Reset after 1 hour
