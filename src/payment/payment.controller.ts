...
export class PaymentController {
  ...
  async createPayment(@Body() paymentData: any) {
    try {
      ...
      if (paymentData.orderId) {
        const order = await this.orderService.findOrderWithItems(parseInt(paymentData.orderId));
        if (!order) {
        ...
        orderDetails = {
          ...
          customer: {
            fullName: order.customer ? `${order.customer.firstname} ${order.customer.lastname}` : 'Guest',
      ...
      if (result.success && result.data && !result.data.checkoutUrl && result.data.orderCode) {
        this.logger.log('Payment link created but missing checkoutUrl, getting payment status');
        
        // Try to get payment status which may contain the checkoutUrl
        const paymentStatus = await this.paymentService.checkPaymentStatus(result.data.orderCode.toString());
        
        if (paymentStatus.success && paymentStatus.data && paymentStatus.data.checkoutUrl) {
          // Add the checkoutUrl from the status response
          result.data.checkoutUrl = paymentStatus.data.checkoutUrl;
          this.logger.log('Retrieved checkoutUrl from payment status');
        } else {
          this.logger.warn('Could not retrieve checkoutUrl from payment status');
        }
      }
  ...
  async checkPaymentStatus(@Param('orderCode') orderCode: string) {
    try {
      this.logger.log(`Checking payment status for order code: ${orderCode}`);
      const result = await this.paymentService.checkPaymentStatus(orderCode);
      
      return result;
    } catch (error) {
      this.logger.error(`Payment status check error: ${error.message}`, error.stack);
      throw new HttpException(
  ...
  @Post('webhook')
  async handlePaymentWebhook(@Body() payload: any, @Headers() headers: any, @Req() request: Request) {
    try {
      ...
      if (result.success) {
        ...
        if (orderId && status === 'PAID') {
          ...
          this.logger.log(`Order ${orderId} updated to PAYMENT_SUCCESS after successful payment`);
        }
      ...
      this.logger.error(`Payment webhook error: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  async handlePaymentSuccess(
    ...
    try {
      this.logger.log(`Processing successful payment callback for order: ${orderId}`);
      this.logger.log(`Payment parameters - status: ${paymentStatus}, code: ${paymentCode}, paymentId: ${paymentId}`);
      ...
      if (!order) {
        this.logger.warn(`Order ${orderId} not found when processing payment success`);
        return {
          success: false,
      ...
      this.logger.log(`Found order ${orderId} with status: ${order.status}`);
      ...
      if (isPaid) {
        ...
        this.logger.warn(`Payment for order ${orderId} has status ${paymentStatus}, code ${paymentCode} - not marking as paid`);
      }
      
      return {
        success: true,
        message: isPaid ? 'Payment successful' : 'Payment status processed',
        orderId,
        paymentStatus
      };
    } catch (error) {
      this.logger.error(`Payment success handler error: ${error.message}`, error.stack);
      return {
        success: false,
  ...
  async debugOrderPaymentStatus(@Param('orderId') orderId: string) {
    try {
      this.logger.log(`Debug request for order payment status: ${orderId}`);
  ...
  async testPaymentSuccess(@Param('orderId') orderId: string) {
    ...
    try {
      this.logger.log(`TEST: Manually updating order ${orderId} to payment_success status`);
