...
export class PCConfigurationController {
    ...
    constructor(private readonly pcConfigurationService: PCConfigurationService) {}
    ...
    async create(@Body() createDto: CreatePCConfigurationDto, @Request() req) {
        try {
            ...
            this.logger.log(`Creating new PC configuration for customer ${customerId}`);
            
            return await this.pcConfigurationService.create(customerId, createDto);
        } catch (error) {
    ...
    async findAll(@Request() req) {
        try {
            ...
            const customerId = req.user.id;
            this.logger.log(`Getting all PC configurations for customer ${customerId}`);
            
            return await this.pcConfigurationService.findAllByCustomerId(customerId);
        } catch (error) {
            this.logger.error(`Error fetching PC configurations: ${error.message}`);
            throw error;
        }
    }
    ...
    async findOne(@Param('id') id: string, @Request() req) {
        try {
            ...
            this.logger.error(`Error fetching PC configuration ${id}: ${error.message}`);
            throw error;
        }
    }
    ...
    async remove(@Param('id') id: string, @Request() req) {
        try {
            ...
            return { success: true, message: 'PC configuration deleted successfully' };
        } catch (error) {
            this.logger.error(`Error deleting PC configuration ${id}: ${error.message}`);
            throw error;
        }
    }
}
