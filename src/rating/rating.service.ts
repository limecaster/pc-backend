...
export class RatingService {
    ...
    async create(
        createRatingCommentDto: CreateRatingCommentDto,
        customerId: number,
    ): Promise<RatingCommentResponseDto> {
        ...
        try {
            ...
            if (existingRatingResult.rows.length > 0) {
                ...
                await pool.query(
                    ...
                    [
                        createRatingCommentDto.stars,
                        createRatingCommentDto.comment,
                        ratingId,
                    ],
                );
            } else {
                ...
                const result = await pool.query(
                    ...
                    [
                        customerId,
                        createRatingCommentDto.productId,
                        createRatingCommentDto.stars,
                        createRatingCommentDto.comment,
                    ],
                );
    ...
    async findAllByProductId(
        ...
        ORDER BY 
          rc.created_at DESC`,
            ...
            throw new BadRequestException(
                `Failed to get ratings: ${error.message}`,
            );
        }
    }
    ...
    async delete(id: number, customerId: number): Promise<void> {
        ...
        try {
            ...
            // Delete the rating
            await pool.query('DELETE FROM "Rating_Comment" WHERE id = $1', [
                id,
            ]);
        } catch (error) {
            if (error instanceof NotFoundException) {
            ...
            throw new BadRequestException(
                `Failed to delete rating: ${error.message}`,
            );
        }
    }
    ...
    async hasUserRated(
        ...
        try {
            ...
            throw new BadRequestException(
                `Failed to check rating status: ${error.message}`,
            );
        }
    }
}
