...
export class WishlistService {
  ...
  async getWishlist(customerId: number): Promise<WishlistItemDto[]> {
    try {
      ...
      for (const item of wishlistItems) {
        ...
        const imageUrl = specificationsResult.records[0]?.get('imageUrl') || '';
  ...
  async addToWishlist(customerId: number, productId: string): Promise<{ message: string }> {
    try {
  ...
  async removeFromWishlist(customerId: number, productId: string): Promise<{ message: string }> {
    try {
  ...
  async isInWishlist(customerId: number, productId: string): Promise<boolean> {
    try {
